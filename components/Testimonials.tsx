  ...
  return (
    <section id="testimonials" className="py-16 md:py-20 bg-gradient-to-b from-[#F8FAFF] via-white to-[#F8FAFF] border-b border-slate-200/80 relative overflow-hidden">
      ...
      <div className="container-main relative z-10 px-4 sm:px-6 md:px-10">
        ...
        {/* ═══ 3D Stage Slider (Compact Width with Generous Margin Space) ═══ */}
        <div className="relative max-w-5xl mx-auto min-h-[440px] sm:min-h-[400px] flex items-center justify-center px-4 sm:px-8">
          
          {/* Navigation Prev Button */}
          <button
            type="button"
            onClick={handlePrev}
            className="absolute left-2 sm:left-4 md:left-6 top-1/2 -translate-y-1/2 z-40 w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-white/95 border-2 border-slate-200/90 text-slate-700 hover:bg-blue-600 hover:text-white hover:border-blue-600 shadow-md hover:shadow-lg flex items-center justify-center transition-all cursor-pointer"
          ...
          <button
            type="button"
            onClick={handleNext}
            className="absolute right-2 sm:right-4 md:right-6 top-1/2 -translate-y-1/2 z-40 w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-white/95 border-2 border-slate-200/90 text-slate-700 hover:bg-blue-600 hover:text-white hover:border-blue-600 shadow-md hover:shadow-lg flex items-center justify-center transition-all cursor-pointer"
            ...
            {TESTIMONIALS.map((item, idx) => {
              ...
              return (
                <div
                  ...
                  className={`absolute transition-all duration-500 ease-out w-[88%] sm:w-[490px] md:w-[550px] lg:w-[590px] rounded-3xl overflow-hidden ${positionStyles}`}
                >
                  <div className="grid grid-cols-1 sm:grid-cols-12 h-full">
                    {/* LEFT SIDE: Name, Firm, Stars, Quote & Stats */}
                    <div className="sm:col-span-7 p-4 sm:p-5 md:p-6 flex flex-col justify-between">
                      <div>
                        {/* Top: Avatar + (Name, Company, Stars in same column, different rows) */}
                        <div className="flex items-start gap-3 mb-3">
                          <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-blue-600 to-blue-700 text-white flex items-center justify-center font-extrabold text-sm shadow-md shadow-blue-500/20 shrink-0 mt-0.5">
                        ...
                        <div className="my-2 py-1">
                          <p className="text-xs sm:text-[13px] md:text-sm text-slate-600 italic leading-relaxed font-medium">
                      ...
                      {item.stats && (
                        <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-xs font-semibold">
                          ...
                          <span className="text-blue-700 font-bold bg-blue-50 px-2.5 py-0.5 rounded-md border border-blue-200/80">
                            {item.stats}
                          </span>
                        </div>
                      )}
                    ...
                    {/* RIGHT SIDE: Video in Portrait Mode */}
                    <div className="sm:col-span-5 p-3.5 sm:p-4 bg-slate-900/5 sm:border-l sm:border-slate-100 flex items-center justify-center">
                      <div className="relative aspect-[3/4] sm:aspect-[4/5] w-full rounded-2xl overflow-hidden bg-slate-950 border-2 border-slate-200 shadow-inner">
                        {isCenter ? (
                          <video
                            controls
                            playsInline
